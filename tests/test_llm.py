...
class TestRegression:
    def test_return_products_in_the_receipt(self, model_client, clear_image_in_b64, report):
        ...
        try:
            ...
            report.false_negative("test_return_products_in_the_receipt failed", answer)
...
class TestHallucination:
    def test_random_b64_string(self, model_client, random_b64_string, report):
        ...
        try:
            ...
            report.false_positive("test_random_b64_string failed", answer)
    ...
    def test_non_receipt_image(self, model_client, random_image_in_b64, report):
        ...
        try:
            ...
            report.false_positive("test_non_receipt_image failed", answer)
...
class TestEdgeCases:
    def test_ripped_receipt(self, model_client, edge_case_image_in_b64_ripped_receipt, report):
        ...
        try:
            ...
            report.false_negative("test_ripped_receipt failed", answer)
    ...
    def test_receipt_from_side(self, model_client, edge_case_image_in_b64_image_from_side, report):
        ...
        try:
            ...
            report.false_negative("test_receipt_from_side failed", answer)
    ...
    def test_minus_sign_in_receipt(self, model_client, edge_case_image_in_b64_minus, report):
        ...
        try:
            ...
            report.false_negative("test_minus_sign_in_receipt failed", answer)

    def test_minus_sign_in_receipt_categorization(self, model_client, edge_case_image_in_b64_minus, report):
        question = """Categorize the products in the receipt that was sent with the prompt into these categories: Food, drinks, 
        Entertainment, Apartment, Electronics, Sports, Other. 
        Return the answer in a list where each item is in json {"product": "product name", "category": "category name", "price": "price"} format. 
        Response should only have this list of json data and nothing else. In the answer price should be in format 0.00. Ignore the KPL at the left of the receipt.
...
class TestPerformance:
    def test_latency_cold_start(self, model_client, clear_image_in_b64, performance_report):
        time.sleep(60)  # Sleep for 60 seconds to simulate cold start
        question = "What products are in the image of receipt that was sent with the prompt?"
        start_time = time.time()
        model_client.generate_answer(clear_image_in_b64, question)
        latency = time.time() - start_time
        performance_report.add_cold_latency(latency)
        

    def test_latency_warm_start(self, model_client, clear_image_in_b64, performance_report):
        question = "What products are in the image of receipt that was sent with the prompt?"
        for _ in range(5): 
            start_time = time.time()
            model_client.generate_answer(clear_image_in_b64, question)
            latency = time.time() - start_time
            performance_report.add_warm_latency(latency)
